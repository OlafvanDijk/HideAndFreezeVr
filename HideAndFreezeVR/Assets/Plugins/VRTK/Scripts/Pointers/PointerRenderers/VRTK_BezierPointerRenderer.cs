...
namespace VRTK
{
    ...
    public class VRTK_BezierPointerRenderer : VRTK_BasePointerRenderer
    {
        ...
        public Vector3 tracerScale = Vector3.one;

        private Vector3? originalBallScale = null;

        private const float cursorYOffset = 0.02f;
        // Improvive END
        ...
        public override void UpdateRenderer()
        {
            ...
            {
                actualCursor.transform.localScale = cursorScale;
            }

            if (actualTracer != null)
            {
                foreach (Transform child in actualTracer.transform)
                {
                    if (originalBallScale == null)
                        originalBallScale = child.localScale;

                    child.localScale = Vector3.Scale(originalBallScale.Value, tracerScale);
                }
            }
            // Improvive END
...
//            Debug.Log("Default cursor scale: " + createdCursor.transform.localScale);
